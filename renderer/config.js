...
const addVarKind = document.getElementById("addVarKind");
const addVarKey = document.getElementById("addVarKey");
const addVarValue = document.getElementById("addVarValue");
const addVarBrowse = document.getElementById("addVarBrowse");
let configName = "";
let configEntries = [];
function isKnownKey(entry) {
    return entry.type === "kv" && !!entry.description;
}
function createPathInput(entry) {
...
function createBoolInput(entry) {
...
function renderCommonSection(common) {
...
function renderOptionalSection(optional) {
    const section = document.createElement("section");
...
function renderDbSection(property, registry) {
    const section = document.createElement("section");
    section.className = "cfg-section";
    const title = document.createElement("h3");
    title.className = "cfg-section-title";
...
function updateAddVarBrowse() {
    if (addVarKind.value === "path")
        addVarValue.placeholder = "C:\\ruta\\...";
    else if (addVarKind.value === "bool")
        addVarValue.placeholder = "true / false";
    else
        addVarValue.placeholder = "Valor";
...
addVarModal.addEventListener("click", (e) => {
    if (e.target === addVarModal)
        closeAddVarModal();
});
function inferConfigMeta(key) {
    const existing = configEntries.find((e) => e.type === "kv" && e.key === key);
    if (existing)
        return { description: existing.description || key, kind: existing.kind || "string" };
