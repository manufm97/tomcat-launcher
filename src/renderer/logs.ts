// src/renderer/logs.ts
// Modal de logs de Tomcat: tail en vivo y selector de ficheros.

const logModal      = document.getElementById("logModal") as HTMLDivElement;
const logViewer     = document.getElementById("logViewer") as HTMLDivElement;
const logFileSelect = document.getElementById("logFileSelect") as HTMLSelectElement;
...
window.api.onLogChunk((text: string) => logAppend(text));
...
async function openLogModal(): Promise<void> {
  ...
  const fillGroup = (label: string, loc: string, files: string[]) => {
  ...
  if (logFileSelect.value && logFileSelect.value !== "__none__") openLogFile();
}
