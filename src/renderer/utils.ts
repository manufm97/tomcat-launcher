...
// Utilidades compartidas: escape HTML, highlight de sintaxis, helpers de UI.

function esc(s: string): string {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

function setChipText(el: HTMLElement, txt: string): void {
  const label = el.querySelector(".chip-label");
  if (label) { label.textContent = txt; } else { el.textContent = txt; }
}

// Patrones estilo "Output Colorizer" (sin grupos capturadores internos).
interface HlPattern { cls: string; re: RegExp; }
