// src/renderer/tests.ts
// Explorador de pruebas del proyecto seleccionado.

const testsModal = document.getElementById("testsModal") as HTMLDivElement;
const testsTree = document.getElementById("testsTree") as HTMLDivElement;
const testsStatus = document.getElementById("testsStatus") as HTMLSpanElement;
const testsDirLabel = document.getElementById("testsDirLabel") as HTMLSpanElement;
const testsProgress = document.getElementById("testsProgress") as HTMLDivElement;
const testsProgressText = document.getElementById("testsProgressText") as HTMLSpanElement;
const testsRefresh = document.getElementById("testsRefresh") as HTMLButtonElement;
const testsRunSelected = document.getElementById("testsRunSelected") as HTMLButtonElement;
const testsRunAll = document.getElementById("testsRunAll") as HTMLButtonElement;
const testsStop = document.getElementById("testsStop") as HTMLButtonElement;
const testsMasterCheck = document.getElementById("testsMasterCheck") as HTMLInputElement;
const testsMasterLabelText = document.getElementById("testsMasterLabel") as HTMLSpanElement;
const summaryPass = document.getElementById("summaryPass") as HTMLElement;
const summaryFail = document.getElementById("summaryFail") as HTMLElement;
const summarySkip = document.getElementById("summarySkip") as HTMLElement;
const summaryNone = document.getElementById("summaryNone") as HTMLElement;

let currentTests: TestNode[] = [];
let selectedIds = new Set<string>();
let currentTestDir = "";
let isTestRunning = false;
let testPollTimer: number | null = null;
const persistedTestStatus = new Map<string, TestNode["status"]>();
let expandedClasses = new Set<string>();

function statusIcon(status: TestNode["status"], isLoading: boolean): string {
...
function statusClass(status: TestNode["status"], isLoading: boolean): string {
  if (isLoading) return "test-running";
  switch (status) {
...
function renderNode(node: TestNode, level: number): HTMLElement {
  ...
  if (node.type === "class" && node.children && node.children.length > 0) {
    ...
    expandBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      if (expandedClasses.has(node.id)) expandedClasses.delete(node.id);
      else expandedClasses.add(node.id);
      renderTests();
    });
  }
  ...
  if (node.type === "class") name.classList.add("test-class-name");
  ...
  if (node.error) row.title = node.error;
...
function hasSelectedChild(node: TestNode): boolean {
  if (node.type === "method") return selectedIds.has(node.id);
  if (!node.children) return false;
  return node.children.some((c) => hasSelectedChild(c));
}

function countSelectedInNode(node: TestNode): number {
  let count = 0;
  if (node.type === "method" && selectedIds.has(node.id)) count++;
  if (node.children) {
    for (const child of node.children) count += countSelectedInNode(child);
  }
  return count;
}

function toggleSelection(node: TestNode, checked: boolean): void {
  if (checked) selectedIds.add(node.id);
  else selectedIds.delete(node.id);
  if (node.children) {
    for (const child of node.children) toggleSelection(child, checked);
  }
}

function updateMasterCheckbox(): void {
...
function updateSummary(): void {
  ...
  function walk(nodes: TestNode[]): void {
...
function countMethods(nodes: TestNode[]): number {
  let count = 0;
  for (const n of nodes) {
    if (n.type === "method") count++;
    if (n.children) count += countMethods(n.children);
  }
  return count;
}

function collectMethodIds(nodes: TestNode[]): string[] {
  const ids: string[] = [];
  for (const n of nodes) {
    if (n.type === "method") ids.push(n.id);
    if (n.children) ids.push(...collectMethodIds(n.children));
  }
  return ids;
}

function getSelectedMethodIds(): string[] {
  const allMethods = new Set(collectMethodIds(currentTests));
  return Array.from(selectedIds).filter((id) => allMethods.has(id));
}

function applyPersistedStatus(nodes: TestNode[]): void {
  for (const node of nodes) {
    const persisted = persistedTestStatus.get(node.id);
    if (persisted) node.status = persisted;
    if (node.children) applyPersistedStatus(node.children);
  }
}

async function refreshTestResults(name: string): Promise<void> {
  try {
...
async function runTests(ids: string[], all = false): Promise<void> {
  const name = select.value;
  if (!name || (!all && ids.length === 0)) return;
  setRunning(true);
...
function openTestsModal(): void {
  testsModal.classList.remove("hidden");
  loadTests();
}

function closeTestsModal(): void {
...
testsMasterCheck.addEventListener("click", (e) => {
  ...
  if (allSelected) {
    ...
    for (const id of collectMethodIds(currentTests)) selectedIds.add(id);
  }
  renderTests();
});
...
function findNode(nodes: TestNode[], key: string): TestNode | null {
  for (const n of nodes) {
    if (n.id === key) return n;
    if (n.type === "class" && n.className === key) return n;
    if (n.children) {
...
window.api.onTestProgress((payload: any) => {
