// src/renderer/new-project.ts
// Modal de nuevo proyecto.

const newProjectModal  = document.getElementById("newProjectModal") as HTMLDivElement;
const newProjectName   = document.getElementById("newProjectName") as HTMLInputElement;
const newProjectJava   = document.getElementById("newProjectJava") as HTMLSelectElement;
const newProjectTomcat = document.getElementById("newProjectTomcat") as HTMLSelectElement;
const addProjectBtn    = document.getElementById("addProjectBtn") as HTMLButtonElement;

const newProjectDocker       = document.getElementById("newProjectDocker") as HTMLInputElement;
const newProjectDockerFields = document.getElementById("newProjectDockerFields") as HTMLDivElement;
const newProjectDockerPath   = document.getElementById("newProjectDockerPath") as HTMLInputElement;
const newProjectDockerPort   = document.getElementById("newProjectDockerPort") as HTMLInputElement;
const newProjectDockerBrowseBtn = document.getElementById("newProjectDockerBrowse") as HTMLButtonElement;

const MANUAL_OPT = "__manual__";

async function populateNewProjectSelects(): Promise<void> {
...
newProjectJava.addEventListener("change", async (e: Event) => {
...
newProjectTomcat.addEventListener("change", async (e: Event) => {
  const target = e.target as HTMLSelectElement;
  if (target.value === MANUAL_OPT) {
    const res = await window.api.browsePath("");
    if (res && !res.canceled && res.path) {
...
function updateNewProjectDockerFields(): void {
...
newProjectDockerBrowseBtn.addEventListener("click", async () => {
  ...
  if (res && !res.canceled && res.path) newProjectDockerPath.value = res.path;
});
...
newProjectModal.addEventListener("click", (e: MouseEvent) => {
...
async function createNewProject(): Promise<void> {
  ...
  let res: { ok: boolean; name?: string; error?: string };
  try {
...
newProjectName.addEventListener("keydown", (e: KeyboardEvent) => {
